#!/usr/bin/env python3
"""
Trích xuất MFCC + Delta (bậc 1) từ file âm thanh WAV.
Delta là đạo hàm bậc 1 theo thời gian, mô tả độ biến thiên của MFCC.
"""
...
# ============ CODE ============
def extract_mfcc_delta(y: np.ndarray, sr: int, cfg: dict) -> np.ndarray:
    """Trích xuất MFCC + Delta (bậc 1) từ tín hiệu âm thanh."""
    ...
    delta = librosa.feature.delta(mfcc, width=cfg.get("width", 9))
    # Ghép MFCC + Delta: (2*n_mfcc, n_frames)
...
def save_config_txt(cfg: dict, output_path: Path) -> None:
    ...
    with open(output_path, "w", encoding="utf-8") as f:
        f.write("# Tham số cấu hình MFCC + Delta\n")
        f.write("=" * 50 + "\n")
...
def main():
    ...
    for wav_path in input_root.rglob("*.wav"):
        ...
        try:
            ...
            features = extract_mfcc_delta(y, sr, cfg)
            np.save(str(out_path), features)
            total += 1
        except Exception as e:
            print(f"  ❌ Lỗi {wav_path.name}: {e}")

    print(f"\n✅ Hoàn tất. Đã trích xuất MFCC+Delta cho {total} file.")
