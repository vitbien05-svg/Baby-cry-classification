#!/usr/bin/env python3
"""
Trích xuất MFCC (Mel-frequency cepstral coefficients) từ file âm thanh WAV.
"""
...
CONFIG = {
    ...
    # Thư mục output: extract/data_extract/mfcc/
    "output_root": Path(__file__).resolve().parent / "data_extract" / "mfcc_40",
    # Tham số MFCC
    "sample_rate": 16000,
    "n_mfcc": 13,
    "n_fft": 512,
...
# ============ CODE ============
def extract_mfcc(y: np.ndarray, sr: int, cfg: dict) -> np.ndarray:
...
def main():
    ...
    for wav_path in input_root.rglob("*.wav"):
        ...
        try:
            ...
            np.save(str(out_path), mfcc)
            total += 1
        except Exception as e:
            print(f"  ❌ Lỗi {wav_path.name}: {e}")

    print(f"\n✅ Hoàn tất. Đã trích xuất MFCC cho {total} file.")
